...
if SidebarButtons["COMBAT"] then
    SidebarButtons["COMBAT"].btn:GetPropertyChangedSignal("Parent"):Connect(function() end)
    for n, p in pairs(Pages) do p.Visible = false end
...
-- Posição será fixada no centro automaticamente no loop
...
local function temParedeNaFrente(char)
    ...
    return ray ~= nil -- true = tem parede (invisível), false = visível
end
    ...
    if State.Aimbot then
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Head") then
                ...
                -- Verifica se tem parede na frente (só se a opção estiver ativa)
